...
import com.bishoy.chapter4.StackX;
...
//--------------------------------------------------------------
	public InToPost(String in) // constructor
	{
		...
		theStack = new StackX<String>(stackSize);
	}
	...
	public String doTrans() // do translation to postfix
	{
		for (int j = 0; j < input.length(); j++) // for each char
		{
			...
			case '(': // it's a left paren
				theStack.push(String.valueOf(ch)); // push it
				break;
...
//--------------------------------------------------------------
	...
	public void gotOper(char opThis, int prec1) { // got operator from input
		while (!theStack.isEmpty()) {
			...
			if (opTop == '(') // if it's a '('
			{
				theStack.push(String.valueOf(opTop)); // restore '('
				break;
			} else // it's an operator
			{
				...
				{ // than prec of old
					theStack.push(String.valueOf(opTop)); // save newly-popped op
					break;
				} else // prec of new not less
					output = output + opTop; // than prec of old
			} // end else (it's an operator)
		} // end while
		theStack.push(String.valueOf(opThis)); // push new operator
	} // end gotOp()
//--------------------------------------------------------------
